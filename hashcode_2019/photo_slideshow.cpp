...
public:
    ...
    int calScore(Photo& b) {
        int score_ab = calIntersect(b);

        int score_a = tags.size() - score_ab;
        int score_b = b.tags.size() - score_ab;
        return min(score_ab, min(score_a, score_b));
    }

    int calMerge(Photo& b) {
        int score_ab = calIntersect(b);

        int score_a = tags.size() - score_ab;
        int score_b = b.tags.size() - score_ab;
        return score_ab + score_a + score_b;
    }
...
int main(int argc, char *argv[]){
    ...
    // merge vertical (smart)
    vector<Photo*> picsVP;
    for(int i = 0; i < pics_v.size(); i++){
        picsVP.push_back(&pics_v[i]);
    }

    while (picsVP.size() >= 2) {
        int mxS = -1;
        int mxI = -1;
        for(int j = 1; j < picsVP.size() && j < 1000; j++){
            int ts = picsVP[0]->calMerge(*picsVP[j]);
        ...
        assert(mxI != -1);
        pSpace.push_back(picsVP[0]->mergePics(*picsVP[mxI]));

        picsVP[mxI] = picsVP.back();
        picsVP.pop_back();

        picsVP[0] = picsVP.back();
        picsVP.pop_back();

        if (pSpace.size() % 100 == 0){
            cout << "Merge " << picsVP.size() << "\n";
        }
    }
    cout << "Done merge\n";
    ...
    while(pSpace.size() > 1){
        ...
        for(int j = 0; j < pSpace.size() && j < 1000; j++){
    ...
    // cal score
    int sumScore = 0;
    for(int i = 1; i < slide.size(); i++){
        sumScore += slide[i-1]->calScore(*slide[i]);
    }
    cout << "Score " << sumScore << "\n";
